...
wait_observability_apps_converged() {
  ...
  while (( elapsed < timeout_seconds )); do
    ...
    if (( pending == 0 )); then
      echo "Validacion final completada: todas las apps estan Synced+Healthy."
      return 0
    fi

    echo "Aun pendientes ${pending} apps (${elapsed}s/${timeout_seconds}s):"
  ...
  echo "Timeout en validacion final. Estado actual de apps de observabilidad:"
  kubectl get applications -n argocd | grep -E "observability|otel|NAME" || true
  return 1
}
